...
このほかにも、TC発言・招待成立・DISBOARD Bump・ブースト・給与・ショップ販売など、Discord上の行動に応じて自動加算される収入があります。
...
現在、公式ショップは全商品を撤去中です（`OFFICIAL_SALE_ITEM_IDS = []`）。既存所持者の表示・使用のためにアイテム定義は残しています。

- 深座りの椅子（旧労働制度の記念品、効果なし）
- 常連カード（ログボ+250）
- くじ付き封筒（ランダム収支）
- 黒金カード枠（プロフィール見栄え）
- 宿の増築券（宿の追加人数枠）
...
- **即時ボーナス**: ブースト1回につきその場でRisを付与します（既定 10,000 Ris、月3回まで。額・上限は運営パネルから変更可）。
