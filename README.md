...
買う側の入口です。公式ショップ（Bot・運営が効果や付与を管理する商品）と、IRIS商店街（ユーザー同士の個人間取引）を分けています。

- IRIS商店街を見る・カテゴリや条件で商品を探す
...
売る側の入口です。店舗の開設操作は不要で、初出品時に販売者ページが自動作成されます。
...
運営が `保守・パネル` で ForumChannel を「IRIS商店街フォーラム」に設定すると、フォーラム = 売り場 / Bot = 決済・取引・エスクロー という構成になります。

- 対象フォーラムに投稿を作ると、Botが「この投稿をIRISで販売しますか？」と案内します
- 販売設定（価格・カテゴリ・受付上限）ができるのは投稿の作成者だけです。商品名は投稿タイトル、説明は本文を使います
- 登録後は投稿内に販売パネル（購入ボタン付き）をピン留めし、同じ商品を `/ショップ` からも購入できます
- 投稿が削除されると新規購入を停止します。フォーラム連携に失敗しても金銭データは影響を受けません
...
運営パネルの「ブースト報酬」から、ブースター自動ロールと枠数ロールを設定します。

- Botが自動判定できるのは「現在ブースト中かどうか」です。
- 個人の1枠/2枠/3枠以上はDiscord APIから直接取れないため、運営が枠数ロールを付け替えます。
- Botは枠数ロールを読み取り、同じ月に1回だけRisを付与します。
- 1枠は10,000 Ris、2枠は20,000 Ris、3枠以上は30,000 Risです。
- 前月もブースト報酬を受け取っている場合、継続ボーナスとして10,000 Risを追加します。
- ブースター自動ロールを設定した場合、Botはブースト開始/終了に合わせてそのロールの付け外しを試みます。枠数ロールは自動変更しません。
...
`npm run smoke` は、参加時の初期資本が素の10万Risになること、カジノ/RPG/シンク/政策コマンドが未知扱いになること、ショップ・オークション・IRIS商店街（エスクロー取引の状態遷移、キャンセル・返金、自動完了、販売者退出、ページ送り、フォーラム連携のガード、旧データmigration）が動作することを検証します。
