...
const commands = [
  ...
  "invite",
  // simulate-text / simulate-vc は本番撤去済みなので commands 配列にも入れない。
  // 下の rank text / rank vc テストで textXp / vcXp が必要なので、後段で activity
  // フィールドを直接 seed する。
  "rank text",
  "rank vc",
  "rank invite",
...
// 旧 simulate-text 20 / simulate-vc 30 の代わりに、当時と同じ活動量を activity へ
// 直接 seed する。本番コード経由ではないため、simulate系撤去の要件と両立する。
// simulate-text は count*12 XP、simulate-vc は minutes*6 XP を加算する仕様だったので
// 同値を入れて downstream の VC rank判定などが元の挙動と一致するようにする。
user.activity.textXp = 20 * 12;
user.activity.textMessages = 20;
user.activity.vcXp = 30 * 6;
user.activity.vcMinutes = 30;
...
// =========================================================================
// 旧収入コマンド撤去 / simulate 無効化 / 椅子返金migration の回帰テスト
// =========================================================================
...
// --- 労働称号「労働市場そのもの」が付与されない ---
{
  ...
  u.workCount = 999;
  u.activity.textXp = 100;
  u.activity.vcXp = 100;
  u.wallet = 5000;
  e.updateTitle(u);
  assert.notStrictEqual(u.title, "労働市場そのもの", "labor titleは付与されてはいけません");
  assert.strictEqual(u.title, "未上場の一般人", "どの条件にも当てはまらない場合はデフォルト称号へ戻す必要があります");
}
...
// --- 旧state互換 ---
{
  ...
  legacyState.users[legacyId] = {
    ...
    xp: 100,
    workCount: 1,
    lastWork: "2026-07-05T13:45:59.377Z",
    lifetimeEarned: 500,
    lifetimeLost: 0,
    inventory: { chair: 1 },
    lastSubsidy: "2026-07-01T00:00:00.000Z",
    title: "労働市場そのもの"
  };
  const e = new EconomyEngine(legacyState, { rng, silentMigrations: true });
  const u = e.state.users[legacyId];
  assert.strictEqual(u.workCount, 1, "旧stateの workCount は保持する必要があります");
  assert.strictEqual(u.lastWork, "2026-07-05T13:45:59.377Z", "旧stateの lastWork は保持する必要があります");
  assert.strictEqual(u.lastSubsidy, "2026-07-01T00:00:00.000Z", "旧stateの lastSubsidy は保持する必要があります");
  assert.strictEqual(u.inventory.chair, 1, "椅子所持は保持される必要があります");
}
...
// 1) 対象者へ1,500 Ris付与 + lifetimeEarned 加算 + 椅子保持 + ledger 記録
{
  ...
  assert.strictEqual(Object.keys(state.migrations[LEGACY_CHAIR_REFUND_MIGRATION_ID].targets).length, 2,
...
// 5) 対象者不在なら新規ユーザーを作らない + 警告ログを出す
{
  const state = createInitialState();
  const warns = [];
  const r = applyLegacyChairRefund(state, { log: (m) => warns.push(m) });
  assert.strictEqual(r.applied.length, 0, "対象者不在: applied 0");
  assert.strictEqual(r.missing.length, LEGACY_CHAIR_REFUND_TARGETS.length, "対象者不在: missing に全員入る");
  assert.strictEqual(Object.keys(state.users || {}).length, 0, "新規ユーザーを作ってはいけません");
  assert(warns.some((m) => m.includes("state に居ないため") || m.includes("居ない")), "警告ログが出る必要があります");
}
...
// 8) EconomyEngine コンストラクタ経由でも migration が適用される
{
  const state = makeChairState();
  const logs = [];
...
// --- help / help内文言のチェック ---
{
  ...
  const helpRes = e.run("help", { id: "help:t", name: "help test" });
  const helpText = JSON.stringify(helpRes);
  for (const term of ["work", "labor", "労働", "subsidy", "beg", "給付金"]) {
    assert(!helpText.includes(term), `help に "${term}" が残っていてはいけません`);
  }
}
